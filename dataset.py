...
PyTorch 本质上是一个深度学习框架，它在底层集成并统一封装了 CPU、CUDA、MPS 等计算后端，对上提供统一且标准的张量与自动求导接口。开发者只需要把数据按照约定组织成 Tensor，
并调用统一的前向、反向与优化 API；而 PyTorch 会依据 Tensor 的 shape、dtype、device 等元信息，自动将计算分发给对应的计算后端，
完成 CPU/GPU 上的高性能并行矩阵运算、自动求导与参数更新，从而屏蔽不同硬件平台和底层实现之间的差异。

Tensor数据结构：
PyTorch 张量 = 数据 + 形状 + 数据类型 + 计算方式
